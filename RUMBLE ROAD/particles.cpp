...
Particles::Particles(float spread, float phase, float speed) {
    ...
    //Custom variables
    spread_ = spread / 100;//divide by 100 to convert from degrees
    phaseConst_ = phase;
    speedConst_ = speed;
...
void Particles::CreateGeometry(void)
{
    ...
    for (int i = 0; i < NUM_PARTICLES; i++){
        ...
        if (i % 4 == 0){
            ...
            theta = (2.0*(rand() % 10000) / 10000.0f -1.0f)*spread_ + pi;
            r = 0.0f + 0.8*(rand() % 10000 * speedConst_) / 10000.0f;
            tmod = (rand() % 10000)/(10000.0f*phaseConst_);
        }
